...
function ausGenerateScriptSignature($my_aus_product_id, $my_aus_product_key)
    {
    $script_signature="";
    ...
    if (!empty($root_ips_array)) //IP(s) resolved successfully
        {
        $script_signature=hash("sha256", gmdate("Y-m-d").$my_aus_product_id.$my_aus_product_key.implode("", $root_ips_array));
        }
...
function ausVerifyServerSignature($notification_server_signature, $my_aus_product_id, $my_aus_product_key)
    {
    $result=false;
    $root_ips_array=gethostbynamel(ausGetRawDomain(AUS_ROOT_URL));

    if (!empty($notification_server_signature) && !empty($root_ips_array) && hash("sha256", implode("", $root_ips_array).$my_aus_product_key.$my_aus_product_id.gmdate("Y-m-d"))==$notification_server_signature) //server signature valid
        {
        $result=true;
        }
...
function ausCheckSettings($my_aus_product_id, $my_aus_product_key)
    {
    ...
    if (!ausValidateIntegerValue($my_aus_product_id)) //invalid AUS product ID
        {
        $notifications_array[]=AUS_CORE_NOTIFICATION_INVALID_PRODUCT_ID;
        }

    if (empty($my_aus_product_key) || $my_aus_product_key=="some_random_key") //invalid AUS product key
        {
...
function ausGetVersion($version_number="", $my_aus_product_id, $my_aus_product_key)
    {
    $notifications_array=array();
    $aus_core_notifications=ausCheckSettings($my_aus_product_id, $my_aus_product_key); //check core settings

    if (empty($aus_core_notifications)) //only continue if script is properly configured
        {
        $post_info="product_id=".rawurlencode($my_aus_product_id)."&product_key=".rawurlencode($my_aus_product_key)."&version_number=".rawurlencode($version_number)."&user_local_path=".rawurlencode(dirname(AUS_DIRECTORY))."&script_signature=".rawurlencode(ausGenerateScriptSignature($my_aus_product_id, $my_aus_product_key));

        $content_array=ausCustomPost(AUS_ROOT_URL."/aus_callbacks/get_version.php", $post_info);
        if (!empty($content_array['headers']['notification_server_signature']) && ausVerifyServerSignature($content_array['headers']['notification_server_signature'], $my_aus_product_id, $my_aus_product_key)) //proper response received
            {
...
function ausGetAllVersions($my_aus_product_id, $my_aus_product_key)
    {
    $notifications_array=array();
    $aus_core_notifications=ausCheckSettings($my_aus_product_id, $my_aus_product_key); //check core settings
    ...
    if (empty($aus_core_notifications)) //only continue if script is properly configured
        {
        $post_info="product_id=".rawurlencode($my_aus_product_id)."&product_key=".rawurlencode($my_aus_product_key)."&user_local_path=".rawurlencode(dirname(AUS_DIRECTORY))."&script_signature=".rawurlencode(ausGenerateScriptSignature($my_aus_product_id, $my_aus_product_key));
...
function ausDownloadFile($license_code, $file_type="version_upgrade_file", $version_number, $my_aus_product_id, $my_aus_product_key)
    {
    $notifications_array=array();
    $aus_core_notifications=ausCheckSettings($my_aus_product_id, $my_aus_product_key); //check core settings
    ...
    if (empty($aus_core_notifications)) //only continue if script is properly configured
        ...
        if (class_exists("ZipArchive")) //proceed to download only if ZipArchive is installed on server
            {
            $post_info="license_code=".$license_code."&product_id=".rawurlencode($my_aus_product_id)."&product_key=".rawurlencode($my_aus_product_key)."&version_number=".rawurlencode($version_number)."&user_local_path=".rawurlencode(dirname(AUS_DIRECTORY))."&file_type=".rawurlencode($file_type)."&script_signature=".rawurlencode(ausGenerateScriptSignature($my_aus_product_id, $my_aus_product_key));
...
function ausFetchQuery($license_code, $query_type, $version_number, $my_aus_product_id, $my_aus_product_key)
    {
    $notifications_array=array();
    $aus_core_notifications=ausCheckSettings($my_aus_product_id, $my_aus_product_key); //check core settings
    ...
    if (empty($aus_core_notifications)) //only continue if script is properly configured
        {
        $post_info="license_code=".$license_code."&product_id=".rawurlencode($my_aus_product_id)."&product_key=".rawurlencode($my_aus_product_key)."&version_number=".rawurlencode($version_number)."&user_local_path=".rawurlencode(dirname(AUS_DIRECTORY))."&query_type=".rawurlencode($query_type)."&script_signature=".rawurlencode(ausGenerateScriptSignature($my_aus_product_id, $my_aus_product_key));
